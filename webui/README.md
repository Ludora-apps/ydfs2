...
JSON API: `GET /api/capabilities`, `GET/POST /api/jobs`, `GET/DELETE /api/jobs/{id}`, `POST /api/jobs/{id}/cancel`, `GET /api/jobs/{id}/events`, `GET /api/jobs/{id}/log`, `GET /api/jobs/{id}/artifacts/{name}`, `GET /api/packages`, `GET /api/packages/{name}`, `GET/PUT /api/profiles`, and `DELETE /api/profiles/{name}`.
