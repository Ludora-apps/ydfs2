...
- A succeeded ISO build can be booted in the browser: **Test in browser** next to a build's artifacts (in Build details and in Kept builds) starts QEMU/KVM in a container from `webui/vm/Dockerfile` (`make vm-image`, tagged `ydfs-vm:1`) with the ISO bind-mounted read-only, and the console appears in a Test machine box over noVNC. One machine at a time, 4 GiB and 4 vCPUs, no disk and no installer: the guest is discarded when it stops. Networking is QEMU user-mode NAT, so the guest reaches the internet and nothing reaches the guest. The container runs unprivileged (`--cap-drop ALL`, `--read-only`, `--security-opt no-new-privileges`) with only `/dev/kvm` passed in; QEMU's VNC server listens on a UNIX socket inside the container and only its websocket port is published, on loopback, with a per-session VNC password, so the console is reachable exclusively through the authenticated `GET /api/vm/console`. That endpoint is a WebSocket, which is a GET and therefore outside the CSRF check, so it verifies `Origin` itself. **This ISO stops at its isolinux boot menu and waits for a keypress** — press Enter or use the toolbar button. A machine stops when you stop it, after 30 minutes with nobody watching, after 15 minutes with no build manager open, after four hours, or when the server shuts down; leftovers are reaped by their `ydfs-vm=1` label at startup and after shutdown. Retention never reclaims the ISO a machine is currently booted from. Booting requires `/dev/kvm` and the image: `GET /api/capabilities` reports `vm` and `vmMessage`, and the button explains itself when unavailable.
